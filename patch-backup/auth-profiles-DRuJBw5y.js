...
async function markAuthProfileFailure(params) {
	...
	const profile = store.profiles[profileId];
	if (!profile) return;
	const isBypassed = isAuthCooldownBypassedForProvider(profile.provider);
	if (isBypassed) {
		await authProfileUsageDeps.updateAuthProfileStoreWithLock({
			agentDir,
			updater: (freshStore) => {
				const fp = freshStore.profiles[profileId];
				if (!fp || !isAuthCooldownBypassedForProvider(fp.provider)) return false;
