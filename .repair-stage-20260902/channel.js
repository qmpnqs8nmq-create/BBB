...
import { notifyStop, notifyStart, getConfig } from "./api/api.js";
...
// Local patch 2026-09-02 (ported from the 2.4.6 patch): attempt a lightweight
// iLink getConfig warm-up before cold/scheduled outbound. This improves error
// visibility and can refresh some stale sessions, but does not bypass Weixin's
// server-side ~24h outbound policy.
async function warmOutboundSession(params) {
    const aLog = logger.withAccount(params.account.accountId);
    try {
        ...
        if (resp.ret !== undefined && resp.ret !== 0) {
            aLog.warn(`${params.label}: getConfig warm-up returned ret=${resp.ret} errmsg=${resp.errmsg ?? ""} for to=${params.to}`);
            return;
        }
        aLog.debug(`${params.label}: getConfig warm-up OK for to=${params.to}`);
    }
    catch (err) {
        // Warm-up is an availability optimization; the send attempt stays the source of truth.
        aLog.warn(`${params.label}: getConfig warm-up failed for to=${params.to}: ${String(err)}`);
    }
}
async function sendWeixinOutbound(params) {
    ...
    await warmOutboundSession({ account, to: params.to, contextToken: params.contextToken, label: "sendWeixinOutbound" });
    try {
        ...
        if (String(err).includes("ret=-2")) {
            // Retry once without a potentially stale context token after a
            // no-context warm-up. The retry remains subject to server policy.
            aLog.warn(`sendWeixinOutbound: sendMessage ret=-2 for to=${params.to}; retrying without stale context_token after no-context warm-up`);
            try {
                await warmOutboundSession({ account, to: params.to, label: "sendWeixinOutbound retry/no-context" });
                const retryResult = await sendMessageWeixin({ to: params.to, text: filteredText, opts: {
                        baseUrl: account.baseUrl,
                        token: account.token,
                        contextToken: undefined,
                    } });
                emitWeixinMessageSent({ to: params.to, content: filteredText, success: true, accountId: account.accountId });
                return { channel: "openclaw-weixin", messageId: retryResult.messageId };
            }
            catch (err2) {
                emitWeixinMessageSent({ to: params.to, content: filteredText, success: false, error: String(err2), accountId: account.accountId });
                throw err2;
            }
        }
